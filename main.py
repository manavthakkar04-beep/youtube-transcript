...
def get_transcript(req: TranscriptRequest):
    ...
    Uses proxy rotation to bypass YouTube IP blocks.
    """
    ...
    # Try with proxies first, then without
    for attempt in range(2):
        try:
            if attempt == 0:
                # First attempt: use proxy to bypass IP blocks
                ...
                # Second attempt: try without proxy
            ...
            if attempt == 0:
                continue  # Try again without proxy
            raise HTTPException(
                status_code=503,
                detail="YouTube has temporarily blocked this IP. Try again later."
            )
    ...
    raise HTTPException(status_code=503, detail="Could not fetch transcript after retries")
